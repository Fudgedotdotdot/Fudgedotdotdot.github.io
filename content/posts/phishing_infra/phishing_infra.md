...
2. [Terraform and Ansible](#terraform-and-ansible)
3. [Evilginx2 configuration](#evilginx2-configuration)
3. [Redirector configuration](#redirector-configuration)
3. [Gophish configuration](#redirector-configuration)
3. [Postfix configuration](#redirector-configuration)
3. [Network and email analysis](#network-and-email-analysis)
3. [Improvements](#mprovements)
...
services:
  evilginx2:
    container_name: seculabs_evilginx2_source_latest
    image: registry.digitalocean.com/seculabs-registry/seculabs-evilginx2
    restart: always
    volumes:
      - /opt/evilginx2/phishlets:/volume/phishlets
      - /opt/evilginx2/redirectors:/volume/redirectors
  ...
  gophish:
    container_name: seculabs_gophish_source_latest
    image: registry.digitalocean.com/seculabs-registry/seculabs-gophish
    restart: always
    ports:
      - "127.0.0.1:8083:3333"
    networks:
...
We will setup another SMTP ssh tunnel like the HTTP one earlier that listens on localhost:25. 
...
I've forked [https://github.com/catatnight/docker-postfix](https://github.com/catatnight/docker-postfix) to add these additional features and pushed the image to [DockerHub](https://hub.docker.com/r/fudgedotdotdot/postfix). You can check out the modification [here](https://github.com/Fudgedotdotdot/docker-postfix/blob/e6b8da74f5d42f4c4c7cc8a6e4f769df6b22174a/assets/install.sh).
...
Authentication-Results: mail.protonmail.ch; arc=none smtp.remote-ip=<ip>
```

For the TLS configuration, running a network capture shows the difference:


<table>
  <tr>
    <td align="center"><strong>TLS</strong></td>
